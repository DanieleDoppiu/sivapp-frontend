...
import { IonContent, IonHeader, IonTitle, IonThumbnail, IonToolbar, IonButton, IonLabel, IonImg, IonItem, IonList, IonIcon, IonTextarea, IonToggle  } from '@ionic/angular/standalone';
...
@Component({
  ...
  standalone: true,
  imports: [IonTextarea, IonIcon, IonList, CommonModule,  FormsModule, RouterModule, IonThumbnail, IonContent, IonHeader, IonTitle, IonToolbar, IonButton, IonLabel, IonImg, IonItem, IonToggle  ]
})
export class AdminDettagliPage implements OnInit {
  ...
  consentiRisposta: boolean = false;
...
inviaMessaggio(consentiRisposta: boolean = false) {
  const testo = this.testoMessaggio.trim();
  if (!testo) return;

  // 🔹 Recupera l'oggetto utente dal localStorage
  const userStr = localStorage.getItem('user');
  let mittenteUsername = 'admin'; // fallback

  if (userStr) {
    try {
      const userObj = JSON.parse(userStr);
      mittenteUsername = userObj.username || 'admin';
    } catch (err) {
  ...
  this.http.post(`${environment.apiUrl}/api/messaggi/admin`, {
    mittenteUsername,                // ora solo lo username
    ...
    next: async () => {
      ...
      toast.present();
      this.testoMessaggio = ''; // pulisce campo testo
    },
    error: async () => {
      const toast = await this.toastCtrl.create({
