...
export class ModificaProfiloPage implements OnInit, AfterViewInit {
...
ngOnInit() {
  ...
  this.http.get<any>(`${environment.apiUrl}/api/utente/${this.userId}`).subscribe({
    ...
    if (username) {
      this.qrData = `https://sivapp.events/calendario-artista?organizzatore=${encodeURIComponent(username)}`;
    }
...
compressImage(dataUrl: string, maxSizeKB: number): Promise<File> {
  ...
  ngAfterViewInit() {
    // genera QR solo se abbiamo dati e canvas
  ...
  // Scarica QR dal canvas
  scaricaQr() {
    if (!this.qrCanvas) return;
    const canvas = this.qrCanvas.nativeElement;
    const link = document.createElement('a');
    link.href = canvas.toDataURL('image/png');
    link.download = `qr-${this.username}.png`;
    link.click();
  }
